...
class Infusionsoft {
	...
	public function __construct( $subdomain = NULL, $api_key = NULL ) {
		...
		if ( empty( $this->subdomain ) || empty( $this->api_key ) ) {
			$this->error = new WP_Error( 'invalid-request', __( 'You must provide a subdomain and API key for your Infusionsoft application.', 'inf-mem' ) );
		}
	}
	...
	public function __call( $name, $arguments ) {
		...
		if ( $this->error ) {
			return new WP_Error( 'invalid-request', __( 'You must provide a subdomain and API key for your Infusionsoft application.', 'inf-mem' ) );
		}
		...
		if ( ! call_user_func_array( array( $client, 'query' ), $arguments ) ) {
            echo '<div id="message" class="error"><p>'.$client->getErrorMessage().'</p></div>';
